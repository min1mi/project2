 ...
 * destroy()
 *   - 딱 한 번만 호출된다.
 *   - 웹 애플리케이션의 실행이 멈출 때, 서블릿을 사용한 자원을 초기화시킬 수 있도록 이 메서드를 호출한다.
 ...
 *    $tmp/stpwebapps/web01/WEB-INF/lib/ 폴더에 복사된다.
 * => 즉 build.gradle의 dependencies {} 블록에 지정된 라이브러리 파일들이 /lib/ 폴더에 복사된다는 것이다.
 * => 주의!
 *   - "servlet-api" 라이브러리는 당연히 서블릿 컨테이너에 포함되어있다.
 *   - 따라서 배치할 필요가 없다.
 ...
 * => 클라이언트 요청을 처리하는 클래스를 만들 때는 반드시 이 규칙에 따라 만들어야 한다.
