/* service() 파라미터 : ServletRequest II - 한글 파라미터 값 깨지는 현상
 ...
 *     URIEncodig="UTF-8"/>
 *     URIEncodig="UTF-8" 속성을 설정한다.
 *   - 즉 GET 요청의 한글 값이 깨지는 경우는 프로그램 코드로 처리할 수 없다.
 *     위와 같이 설정으로 처리해야 한다.
 *     
 * => POST 요청의 파라미터 값 깨지는 경우
 *   - 프로그램 코드로 처리할 수 있다.
 *   - 파라미터 값을 꺼내기 전에, 즉 getParameter()를 최초로 호출하기 전에 클라이언트의 값이 어떤 문자집합으로
 *     인코딩 되었는지 알려준 후에 꺼낸다.
 *     예) req.setCharacterEncoding("UTF-8");
 *     
 *   - 깨지는 이유?
 *     서블릿 컨테이너는 클라이언트가 보내는 값이 ISO-8859-1로 인코딩 되어 있을 거라 가정한다.
 ...
 *            00 48 00 42 00 43 00 EA 00 B0 00 80 00 EA 00 B0 00 81 00 EA 00 B0 00 84 
 *         => 제대로 유니코드로 바꾼다면 다음과 같아야 한다. (UTF-9 --> Unicode)
